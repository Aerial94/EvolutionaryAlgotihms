...
SimpleEvolutionaryAlgorithm::SimpleEvolutionaryAlgorithm(std::shared_ptr<Problem> problem) : IEvolutionaryAlgorithm(problem) {
    currentPopulation = 0;
...
SimpleEvolutionaryAlgorithm::crossover(std::vector<std::shared_ptr<IElement>> &tempPopulation) {
    ...
    std::uniform_int_distribution<int> intDistribution(0 ,tempPopulation.size());
    for (std::shared_ptr<IElement> &element: tempPopulation) {
        if (doubleDistribution(generator) < CROSSOVER_CHANCE) {
